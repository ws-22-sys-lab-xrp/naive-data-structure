                                ...
                                uint64_t key_offset_2, uint64_t key_len_2)
{
    ...
    return ((key_len_1 == key_len_2) ? 0 : (key_len_1 < key_len_2) ? -1
                                                                   : 1);
}
                                    ...
                                    uint64_t *offset, uint64_t *size, uint64_t *p_delta)
{
    ...
    b = *(p_base + (p_offset & EBPF_CONTEXT_MASK));
    cell_desc = b;
    if ((ebpf_get_cell_type(cell_desc) != EBPF_CELL_ADDR_INT && ebpf_get_cell_type(cell_desc) != EBPF_CELL_ADDR_LEAF && ebpf_get_cell_type(cell_desc) != EBPF_CELL_ADDR_LEAF_NO) || ((cell_desc & EBPF_CELL_64V) != 0))
    {
        return -EBPF_EINVAL;
    }
    ++p_offset;
                                   ...
                                   uint64_t *key_offset, uint64_t *key_size, uint64_t *p_delta)
{
    ...
    /* read the first cell descriptor byte (cell type, RLE count) */
    b = *(p_base + (p_offset & EBPF_CONTEXT_MASK));
    if ((ebpf_get_cell_type(b) != EBPF_CELL_KEY) || ((b & EBPF_CELL_64V) != 0))
    {
        return -EBPF_EINVAL;
    }
    ++p_offset;
